...
int parallel_thread_signal ( parallel_thread *p_parallel_thread, int signal )
{
    // Argument check
    ...
    #else
        if ( pthread_kill(p_parallel_thread->platform_dependent_thread, signal) ) goto failed_to_cancel_parallel_thread;
    #endif
